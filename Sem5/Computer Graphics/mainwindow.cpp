    ...
    if(gridsize == 1) img.setPixel(x,y,qRgb(r,g,b));
        ...
        for(int i=x+1;i <  x + gridsize;i++) {
            for(int j=y+1;j < y + gridsize;j++) {
                if (i < ui->frame->height() && j < ui->frame->width())
...
void MainWindow::on_show_axes_clicked()
{
    if(ui->show_axes->isChecked())
    {
        ...
        {
            point(img.width()/2,j,128,128,128);
        }
        for(int i=0;i<ui->frame->height();i+=gridsize)
        {
            point(i,img.height()/2,128,128,128);
        }
    }
    else{
...
void MainWindow::on_setgridbutton_clicked()
{
    ...
    gridsize=ui->gridsizeInput->value();
    if(gridsize>1)
    {
...
void MainWindow::DDAline(int r=255, int g=255, int b=0)
{
...
//        point(x + gridsize/2.0,y + gridsize/2.0,r,g,b);
        delay();
        point(x,y,r,g,b);
    ...
    ui->exec_time->setText(QString::number(duration.count()) + " \u03BCs");
}
...
{
    if (ui->draw_circle->isChecked() == false)
        return;
    auto start = high_resolution_clock::now();
...
void MainWindow::on_polarCircle_clicked()
{
    ...
    while (theta <= 360.0) {
        ...
        point(xk, yk);
        start = high_resolution_clock::now();
        xk = x0 + r * cos(M_PI * theta / 180) * gridsize;
        yk = y0 + r * sin(M_PI * theta / 180) * gridsize;
        theta += 1.0 / r;
        stop = high_resolution_clock::now();
        intervals += (stop - start);
    }
...
{
    if (ui->draw_ellipse->isChecked() == false)
        return;
    auto start = high_resolution_clock::now();
    int rx = ui->ellipse_radius_1->value();
    int ry = ui->ellipse_radius_2->value();
    p1.setX(ui->frame->x);
    p1.setY(ui->frame->y);
    ...
    for (int i = 0; i <= 360; ++i) {
        ...
        xk = x0 + rx * cos(theta) * gridsize;
        yk = y0 + ry * sin(theta) * gridsize;
        stop = high_resolution_clock::now();
        intervals += (stop - start);
    }
    auto duration = duration_cast<microseconds>(intervals);
...
void MainWindow::on_midpointEllipse_clicked()
{
    ...
    //For first region
    double d1=ry2-rx2*ry+(0.25)*rx2; //Initial value of decision paramemter
    auto stop = high_resolution_clock::now();
    auto intervals = (stop - start);
    while(px<py)
    {
    ...
    start = high_resolution_clock::now();
    //For second region
    double   d2=ry2*((double)x+0.5)*((double)x+0.5)+rx2*(y-1)*(y-1)-rx2*ry2; //Initial value of decision paramemter
    stop = high_resolution_clock::now();
    intervals += (stop - start);
    while(y>=0)
    {
...
void MainWindow::boundaryFillutil(int x, int y, QRgb edgecolour, int r, int g, int b) {
    ...
    boundaryFillutil(x, y + gridsize, edgecolour,r,g,b);
    if (ui->eightPointSymmetry->isChecked()) {
...
void MainWindow::floodFillutil(int x, int y, QRgb oldcolor, int r, int g, int b) {
    if(x < 0 || y < 0 || x >= img.width() || y >= img.height()) return;
    if(img.pixel(x,y) != oldcolor) return;
    delay();
    point(x,y,r,g,b);
    floodFillutil(x - gridsize, y, oldcolor,r,g,b);
    floodFillutil(x + gridsize, y, oldcolor,r,g,b);
    floodFillutil(x, y - gridsize, oldcolor,r,g,b);
    floodFillutil(x, y + gridsize, oldcolor,r,g,b);
    if (ui->eightPointSymmetry->isChecked()) {
        floodFillutil(x - gridsize, y - gridsize, oldcolor,r,g,b);
        floodFillutil(x + gridsize, y + gridsize, oldcolor,r,g,b);
        floodFillutil(x + gridsize, y - gridsize, oldcolor,r,g,b);
        floodFillutil(x - gridsize, y + gridsize, oldcolor,r,g,b);
    }
...
void MainWindow::on_floodFill_clicked()
{
    ...
    y = y*gridsize + gridsize/2;
    point(x,y,0,0,0);
    floodFillutil(x,y,qRgb(0,0,0),0,0,255);
}

vector<pair<int, int>> vertex_list;

void MainWindow::on_setPolygonVertex_clicked()
{
    int x = ui->frame->x/gridsize;
    int y = ui->frame->y/gridsize;
    x = x*gridsize + gridsize/2;
    y = y*gridsize + gridsize/2;
    vertex_list.push_back({x, y});
    int pts = vertex_list.size();
    if (pts >= 2) {
        storeEdgeInTable(vertex_list[pts-2].first, vertex_list[pts-2].second, vertex_list[pts-1].first, vertex_list[pts-1].second);//storage of edges in edge table.
        p1.setX(vertex_list[pts-1].first);p1.setY(vertex_list[pts-1].second);
        p2.setX(vertex_list[pts-2].first);p2.setY(vertex_list[pts-2].second);
        on_DDALine_clicked();
    }
}
...
// TRANSFORMATION
void MainWindow::poly_draw(vector<pair<int,int>> vlist, int r, int g, int b) {
    ++r;--r;++g;--g;++b;--b;  // to remove warnings, will use rgb later maybe
...
{
    double shx = (ui->xshear->text()).toDouble();
    double shy = (ui->yshear->text()).toDouble();
    gridsize = ui->gridsizeInput->value();
